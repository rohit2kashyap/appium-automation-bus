...
function gesturesChainToString(gestures, keysToInclude = ['options']) {
    return gestures
        .map((item) => {
        let otherKeys = lodash_1.default.difference(lodash_1.default.keys(item), ['action']);
        otherKeys = lodash_1.default.isArray(keysToInclude) ? lodash_1.default.intersection(otherKeys, keysToInclude) : otherKeys;
        if (otherKeys.length) {
            return (`${item.action}` +
                `(${lodash_1.default.map(otherKeys, (x) => x + '=' + (lodash_1.default.isPlainObject(item[x]) ? JSON.stringify(item[x]) : item[x])).join(', ')})`);
        }
        return item.action;
    })
        .join('-');
...
function assertNoWebElements(actionSeq) {
    const isOriginWebElement = (gesture) => lodash_1.default.isPlainObject(gesture) && 'origin' in gesture && JSON.stringify(gesture.origin).includes(':wdc:');
    const hasWebElements = actionSeq
        .some((action) => (action?.actions || []).some(isOriginWebElement));
    if (hasWebElements) {
        throw new driver_1.errors.InvalidArgumentError(`The XCUITest driver only supports W3C actions execution in the native context. ` +
